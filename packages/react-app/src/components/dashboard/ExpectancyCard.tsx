  ...
  return (
    ...
    >
      <div className="flex flex-col">
        <div className={`text-2xl font-semibold font-mono ${value >= 0 ? 'text-green-500' : 'text-red-500'}`}>
          {value >= 0 ? '+' : ''}<CountUp end={value} separator="," decimal="." decimals={2} duration={1} prefix="$" />
        </div>
        
        <div className="flex items-center gap-1 mt-1">
          <span className="text-xs text-gray-500 dark:text-gray-400">Per trade profit/loss</span>
          <span className={`text-xs font-medium ${value >= 0 ? 'text-green-500' : 'text-red-500'}`}>
