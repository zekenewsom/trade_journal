import { MetricCard } from '../ui/MetricCard';
import CountUp from 'react-countup';
import { ArrowUp } from 'lucide-react';

interface UnrealisedPnLCardProps {
  value: number;
  change: number;
  changePercentage: number;
}

export function UnrealisedPnLCard({
  value = 47892.21,
  change = 3200.21,
  changePercentage = 7.15
}: Partial<UnrealisedPnLCardProps>) {
  const isPositive = value >= 0;
  const statusColor = isPositive ? 'good' : 'bad';
  
  return (
    <MetricCard title="Unrealised P&L" status={statusColor}>
      <div className="flex flex-col">
        <div className={`text-2xl font-semibold font-mono ${isPositive ? 'text-positive' : 'text-negative'}`}>
          {isPositive ? '+' : ''}
          $<CountUp end={value} separator="," decimals={2} preserveValue />
        </div>
        
        {change !== 0 && (
          <div className="flex items-center gap-1.5 mt-1 text-xs text-gray-400">
            <span>vs previous day:</span>
            <div className="flex items-center gap-1">
              <ArrowUp size={10} className={isPositive ? 'text-positive' : 'text-negative rotate-180'} />
              <span className={isPositive ? 'text-positive' : 'text-negative'}>
                {Math.abs(changePercentage).toFixed(2)}%
              </span>
            </div>
          </div>
        )}
        
        <div className="mt-2">
          <div className="w-full bg-dark-800 h-1.5 rounded-full overflow-hidden">
            <div 
              className={`h-full ${isPositive ? 'bg-positive' : 'bg-negative'}`}
              style={{ width: `${Math.min(Math.abs(changePercentage) * 4, 100)}%` }}
            />
          </div>
        </div>
      </div>
    </MetricCard>
  );
} 