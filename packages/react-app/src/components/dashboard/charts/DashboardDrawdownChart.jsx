...
const DashboardDrawdownChart = ({ equityCurveData }) => {
    const drawdownData = useMemo(() => {
        if (!Array.isArray(equityCurveData) || equityCurveData.length === 0) return [];
        // Ensure all points have an equity property
        const validData = equityCurveData.filter(point => point && typeof point.equity === 'number');
        if (validData.length === 0) return [];
        let peak = validData[0].equity;
