...
const LogTransactionForm = ({ onSubmit, initialValues }) => {
    ...
    const validateForm = () => {
        ...
        if (!quantityStr || quantityStr === '' || isNaN(quantityNum) || quantityNum <= 0) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let userMessage = 'An unexpected error occurred while logging the transaction.';
            if (err && typeof err === 'object') {
                // Network error detection (example: fetch or axios)
                if (err.name === 'TypeError' && /network/i.test(err.message)) {
                    userMessage = 'Network error: Please check your internet connection and try again.';
                } else if ('message' in err && typeof err.message === 'string' && err.message.trim() !== '') {
                    userMessage = err.message;
                }
            }
            setSubmissionStatus({ message: `Error: ${userMessage}`, type: 'error' });
        }
        finally {
            setSubmitting(false);
        }
