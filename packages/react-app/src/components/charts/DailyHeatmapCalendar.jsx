...
function CalendarDay({ date, pnl, isCurrentMonth, maxPnl, getCellColor, colors, typography, br, spacing, alpha, formatCurrency }) {
    return (
        <Tooltip
            ...
            >
                <Typography
                    ...
                    sx={{
                        ...
                        color: isCurrentMonth ? (Math.abs(pnl) > 0.5 * maxPnl ? colors.onPrimary : colors.textSecondary) : alpha(colors.textSecondary, 0.7),
...
export function DailyHeatmapCalendar({ data, targetMonth = new Date(), height = '100%' }) {
    ...
    return (
        <Box sx={{ height, display: 'flex', flexDirection: 'column' }}>
            <Box display="grid" gridTemplateColumns="repeat(7, 1fr)" gap={spacing['0.5']} mb={spacing['1']}>
                {weekDays.map(day => (
                    <Typography
                        variant="caption"
                        align="center"
                        sx={{ color: colors.textSecondary, fontSize: typography.fontSize.xxs }}
                        key={day}
                    >
                        {day}
                    </Typography>
            ...
            <Box display="grid" gridTemplateColumns="repeat(7, 1fr)" gap={spacing['0.5']} flexGrow={1}>
